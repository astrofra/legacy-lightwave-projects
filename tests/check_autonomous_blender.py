...
def worker(package, report, renders, reference=None):
    ...
    if reference:
        for frame in frames:
            ...
            if path.is_file(): native_frames[frame["frame"]]=read_capture(path)
        if not native_frames: raise ValueError('No native reference frame overlaps the C bake')
    records=[]
    ...
    for entry in manifest["gltf_rigs"]:
        ...
        errors=[];native_errors=[];first=None;movement=0;camera=None;expected_first=None;expected_movement=0
        for frame in frames:
            ...
            for actual,(point,position,weights) in zip(points,rows):
                ...
                expected_points.append(Vector((expected[0],-expected[2],expected[1])))
                errors.append((actual-Vector((expected[0],-expected[2],expected[1]))).length)
                if frame["frame"] in native_frames:
            ...
            if expected_first is None: expected_first=expected_points
            movement=max(movement,max((a-b).length for a,b in zip(points,first)))
            expected_movement=max(expected_movement,max((a-b).length for a,b in zip(expected_points,expected_first)))
        ...
        record={"gltf":str(path),"gltf_sha256":digest(path),"bin_sha256":digest(package/entry["gltf_bin"]),
                ...
                "maximum_blender_vertex_movement":movement,"maximum_expected_vertex_movement":expected_movement,
                "has_visible_motion":expected_movement>1e-3,"passed":max(errors)<1e-5 and abs(movement-expected_movement)<2e-5}
        records.append(record)
        if native_errors:
            record["native_cage_comparison"]={"frames":str(reference),"sampled_frames":sorted(native_frames),"maximum_vertex_error":max(native_errors),"rms_vertex_error":math.sqrt(sum(x*x for x in native_errors)/len(native_errors)),"scope":"Difference from native deformed cage on available reference frames, including both IK and weight approximation; no equivalence asserted"}
