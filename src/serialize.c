...
int lw_write_object(const char *dir,const LWObject *o,LWError *e) {
    ...
    for(i=0;i<o->materials.n;i++) {
        ...
        fputs(",\"derived_maps\":{",f);
        {
            ...
            fputs("},\"derived_map_sha256\":{",f); comma=0;
            for(j=0;j<6;j++) if(uris[j]) { if(comma) fputc(',',f); comma=1; lw_json_string(f,keys[j]); fputc(':',f); lw_json_string(f,m->texture_sha256[j]); }
        }
        fputs("}}",f);
    }
    ...
    fprintf(f,",\n\"buffer_bytes\":%zu,\"invalid_map_references\":%zu,\"missing_materials\":%zu,\"non_finite_map_values\":%zu,\"material_scope\":\"scalar subset, LWOB texture bindings and LWO2 UV image layers; native projections and source SURF/CLIP bytes retained; derived PNG maps use a repeat sampler and approximate scalar interpolation; height bump exported to OBJ only\"\n}\n",offset,o->invalid_map_references,o->missing_materials,o->non_finite_map_values);
